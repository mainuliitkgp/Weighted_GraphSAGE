...
flags.DEFINE_string('train_prefix', './Toy_dataset/toy', 'name of the object file that stores the training data. must be specified.')
flags.DEFINE_string('weighted_adjacency_matrix_file', './Toy_dataset/toy-weighted_adjacency_matrix.npy', 'name of the object file that stores the weighted adjacency matrix. must be specified.')

# left to default values in main experiments 
flags.DEFINE_integer('epochs', 100, 'number of epochs to train.')
flags.DEFINE_float('dropout', 0.0, 'dropout rate (1 - keep probability).')
flags.DEFINE_float('weight_decay', 0.0, 'weight for l2 loss on embedding matrix.')
flags.DEFINE_integer('max_degree', 3, 'maximum node degree.')
flags.DEFINE_integer('samples_1', 3, 'number of samples in layer 1')
flags.DEFINE_integer('samples_2', 2, 'number of users samples in layer 2')
flags.DEFINE_integer('dim_1', 1, 'Size of output dim (final is 2x this, if using concat)')
flags.DEFINE_integer('dim_2', 1, 'Size of output dim (final is 2x this, if using concat)')
...
def evaluate(sess, model, minibatch_iter, layer_infos, size=None):
    t_test = time.time()
    feed_dict_val = minibatch_iter.val_feed_dict(layer_infos, size)
...
def save_val_embeddings(sess, model, minibatch_iter, size, out_dir, layer_infos, mod=""):
    val_embeddings = []
    finished = False
    seen = set([])
    nodes = []
    ...
    name = "val"
    while not finished:
        feed_dict_val, finished, edges = minibatch_iter.incremental_embed_feed_dict(size, iter_num, layer_infos)
        iter_num += 1
...
def construct_placeholders():
    ...
    placeholders = {
        ...
        'samples1' : tf.placeholder(tf.int32, shape=(None, None), name='samples1'),
        'support_sizes1' : tf.placeholder(tf.int32, shape=(None), name='support_sizes1'),
        'samples2' : tf.placeholder(tf.int32, shape=(None, None), name='samples2'),
        'support_sizes2' : tf.placeholder(tf.int32, shape=(None), name='support_sizes2'),
        'neg_samples' : tf.placeholder(tf.int32, shape=(None, None), name='neg_samples'),
        'neg_support_sizes' : tf.placeholder(tf.int32, shape=(None), name='neg_support_sizes'),
    }
    return placeholders
        ...
        if FLAGS.model == "n2v":
            ...
            for epoch in range(FLAGS.n2v_test_epochs):
                ...
                while not test_minibatch.end():
                    feed_dict = test_minibatch.next_minibatch_feed_dict(layer_infos)
                    feed_dict.update({placeholders['dropout']: FLAGS.dropout})
